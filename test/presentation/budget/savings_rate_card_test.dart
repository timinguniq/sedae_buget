...
void main() {
  testWidgets('shows rate, peer rate and encouragement when below peer', (t) async {
    await t.pumpWidget(_wrap(const SavingsRateCard(label: '이번 달', rate: 18, peerRate: 23)));
    await t.pump();
  ...
  testWidgets('praises when at or above peer', (t) async {
    await t.pumpWidget(_wrap(const SavingsRateCard(label: '이번 달', rate: 30, peerRate: 23)));
    await t.pump();
  ...
  testWidgets('지출이 소득보다 많으면 음수 저축률을 그대로 보인다', (t) async {
    await t.pumpWidget(_wrap(const SavingsRateCard(label: '8월', rate: -20, peerRate: 23)));
    await t.pump();
